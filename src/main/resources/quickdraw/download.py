...
GSUTIL = r"C:\Users\njwwn\AppData\Roaming\Python\Python314\Scripts\gsutil"
...
for i, keyword in enumerate(keywords):
    filename = f"{keyword}.ndjson"
    if os.path.exists(filename):
        print(f"[{i+1}/{len(keywords)}] 이미 존재, 스킵: {filename}")
        continue

    src = f"gs://quickdraw_dataset/full/simplified/{keyword}.ndjson"
    print(f"[{i+1}/{len(keywords)}] 다운로드 중: {filename}")
    result = subprocess.run([GSUTIL, "cp", src, "."], capture_output=True)
